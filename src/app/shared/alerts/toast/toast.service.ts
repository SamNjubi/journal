...
export class ToastService {
    ...
    constructor(
        ...
        this.toastConfig.opacity = use(this.toastConfig.opacity, this.defaultOpacity);
        ...
        this.toastConfig.iconClasses.success =
            this.toastConfig.iconClasses.success || 'toast-success';
        this.toastConfig.iconClasses.warning =
            this.toastConfig.iconClasses.warning || 'toast-warning';
    ...
    show(message: string, title?: string | any, override?: IndividualConfig | any, type = ''): ActiveToast | null | any {
        if (this.toasts.filter(t => t.message === message).length === 0) {
            const config = override ? this.applyConfig(override) : this.applyConfig({});
            const toastType = type.includes('toast') ? type : `toast-${type}`;
            return this._buildNotification(toastType, message, title, config);
        }
    }

    /** show successful toast */
    success(message: string, title?: string | any, override?: IndividualConfig): ActiveToast | null | any {
        if (this.toasts.filter(t => t.message === message).length === 0) {
    ...
    /** show error toast */
    error(message: string, title?: string | any, override?: IndividualConfig): ActiveToast | null | any {
        if (this.toasts.filter(t => t.message === message).length === 0) {
    ...
    /** show info toast */
    info(message: string, title?: string | any, override?: IndividualConfig): ActiveToast | null | any {
        if (this.toasts.filter(t => t.message === message).length === 0) {
    ...
    /** show warning toast */
    warning(message: string, title?: string | any, override?: IndividualConfig): ActiveToast | null | any {
